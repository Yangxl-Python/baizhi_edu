...
class Course(BaseModel):
    ...
    def chapter_list(self):
        ...
        chapter_list = CourseChapter.objects.filter(is_show=True, is_delete=False, course_id=self.id)
    ...
    @staticmethod
    def compute_price(price, active_list):
        ...
        if price >= condition:
            ...
            if sale == "":
                ...
                if price_list:
                    price = price - max(price_list)  # 课程原价减去当前满足条件的最大优惠
        return price
    ...
    def real_price(self):
        ...
        if len(active_list) > 0:
            ...
            return float(f'{self.compute_price(price, active_list):.2f}')
    ...
    def expire_list(self):
        """获取课程有效期"""
        expires = self.course_expire.filter(is_show=True, is_delete=False).order_by('orders', 'id')
        # print(expires)
    ...
    def real_expire_price(self, expire_time):
        """点击有效期后的价格、
        ...
        try:
            if expire_time > 0:
                # 如果有效期大于0  则返回有效期所对应的价格
                original_price = CourseExpire.objects.get(course_id=self.id, expire_time=expire_time).price
        ...
        if len(active_list) > 0:
            price = self.compute_price(original_price, active_list)
...
class CourseChapter(BaseModel):
    ...
    @property
    def lesson_list(self):
        lesson_list = CourseLesson.objects.filter(is_show=True, is_delete=False, chapter_id=self.id)
