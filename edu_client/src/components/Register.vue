      ...
      <div class="register_box">
        ...
        <div class="inp">
          ...
          <div class="sms-box">
            ...
            <div class="sms-btn" @click="get_code" :disabled="is_send">{{sms_btn}}</div>
          </div>
          <button class="register_btn" @click="register" :disabled="is_sub">{{register_btn}}</button>
    ...
    methods: {
      register() {
        ...
        this.$axios({
          ...
          this.is_sub = false;
          this.register_btn = '注册';
          let err_data = error.response.data.non_field_errors;
          if (err_data) {
            this.$message({message: err_data[0], type: 'error', showClose: true});
          } else {
      ...
      check_phone() {
        if (this.phone) {
          this.$axios.get(this.$settings.HOST + `user/check_phone/${this.phone}/`).catch(error => {
            this.$message({message: error.response.data.message, type: 'error', showClose: true});
          });
        }
      },
      get_code() {
